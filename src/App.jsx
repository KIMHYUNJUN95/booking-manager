  ...
  return (
    <div className="dashboard-content">
      <div className="dashboard-header"><h2 className="page-title" style={{color:'#5856D6'}}>🛏️ 숙박 현황 (Stay Month)</h2>
        <div style={{display:'flex', alignItems:'center', gap:'10px'}}><span style={{fontSize:'14px', fontWeight:'600', color:'#86868B'}}>조회할 숙박 월:</span><input type="month" className="month-select" value={targetMonth} onChange={e=>setTargetMonth(e.target.value)} /></div>
      </div>
      ...
      <div className="dashboard-header"><h2 className="page-title">전체 기록 관리</h2>
        <div style={{display:'flex', alignItems:'center', gap:'10px'}}>
          <select className="month-select" value={selectedBuilding} onChange={(e) => { setSelectedBuilding(e.target.value); setSelectedRoom("전체"); }}>
            <option value="전체">전체 건물</option>{Object.keys(BUILDING_DATA).map(b => <option key={b} value={b}>{b}</option>)}
          </select>
